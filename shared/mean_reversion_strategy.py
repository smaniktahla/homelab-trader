...
interface (Callable[[list[Bar]], str | None]) -- makes mean_reversion
available as a pure bars_seen -> signal function the same shape as
shared/bollinger_breakout_strategy.py / shared/ema_crossover_strategy.py,
for any research script (VR-3a's sizing comparison harness, or similar)
that needs to run mean_reversion's core entry rule through
shared/backtest_engine.py alongside those two.
