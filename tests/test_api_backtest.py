...
def test_run_backtest_with_daily_8ema_pullback_strategy(api_client, conn):
    _seed_price_history(conn, SYMBOL, n=50)
    r = api_client.get(f"/api/backtest/daily_8ema_momentum_retest/{SYMBOL}?days=60", auth=AUTH)
    assert r.status_code == 200, r.text
    body = r.json()
    assert {o["name"] for o in body["overlays"]} == {"ema_8"}
